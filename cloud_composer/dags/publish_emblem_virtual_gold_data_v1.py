...
dag = DAG(
    dag_id="publish_emblem_virtual_gold_data_v1",
...
push_claims_encounters = KubernetesPodOperator(
    ...
    cmds=[
        ...
        "--sourceDataset=gold_claims_virtual",
    ],
    dag=dag,
)
...
dbt_run_cmd = """
    dbt deps &&
    dbt run --model source:emblem_silver_virtual+ source:emblem_virtual+ --exclude tag:nightly+ abstractions.computed
"""
...
email_stakeholders = EmailOperator(
    ...
    subject="Airflow: Emblem virtual data published for shard: {{ task_instance.xcom_pull(task_ids='get_latest_drop_date') }}",
