...
environment = "prod"
aptible_database_name = "commons-production"
gcs_export_bucket = "cbh-export-commons-prod"
database_name = "prod"
gcp_project = "cbh-db-mirror-prod"
bq_output_dataset = "commons_mirror"
svc_acct = "prod-commons-mirror@cbh-db-mirror-prod.iam.gserviceaccount.com"
...
gcp_secrets = secret.Secret(
    ...
    secret="tf-svc-prod-commons-mirror",
    key="key.json",
)
...
dag = DAG(
    dag_id="prod_commons_mirroring_v4",
    start_date=datetime.datetime(2021, 1, 19, 17),  # 12AM EST (1AM EDT)
...
update_bq_views = KubernetesPodOperator(
    ...
    task_id="update-bq-views",  # rare kebab case
...
calculate_coverage_sli = KubernetesPodOperator(
    dag=dag,
    name="calculate-coverage-sli",
    task_id="calculate_coverage_sli",
    ...
    cmds=[
        "python",
        "./db_mirror/coverage_metric.py",
        "--db=commons",
        f"--project={SLI_PROJECT}",
        "--shard={{ next_ds_nodash }}",
        f"--dataset-id={SLI_DATASET}",
    ],
)

aptible_to_gcs_export >> scio_mirroring_job
scio_mirroring_job >> [update_bq_views, calculate_coverage_sli]
