...
PARTNER_PROJECT = "tufts-data"

silver_tables = ["Provider", "PharmacyClaim", "MedicalClaim", "Member"]
gold_tables = ["Member", "Pharmacy", "Provider"]
...
dag = DAG(
    dag_id="publish_tufts_monthly_gold_data_v1",
...
dbt_run_cmd = """	
    dbt deps &&	
    dbt run --model source:tufts+ source:tufts_silver+ --exclude tag:nightly+ abstractions.computed	
"""
...
email_stakeholders = EmailOperator(
    ...
    subject="Airflow: Tufts data published for shard: {{ task_instance.xcom_pull(task_ids='get_latest_drop_date') }}",
