...
"""PROD: Incrementally exports Zendesk models via Zendesk API incremental endpoints.

DAG: The tasks are carried out by a `KubernetesPodOperator`
...
dag = DAG(
    dag_id="prod_zendesk_incremental_export_v1",
    description="Incrementally export Zendesk models via incremental endpoints",
    # Should run every hour starting at 12:00PM EST / 17:00PM UTC.
...
def _get_k8_operator_for_model(model_name: str):
    return KubernetesPodOperator(
        name=f"prod-zendesk-incremental-export-{model_name}",
        task_id=f"prod_zendesk_incremental_export_{model_name}",
        namespace="default",
...
PUBUSUB_TOPIC = "zendeskTicket"
publish_tickets_to_pubsub = KubernetesPodOperator(
    name="prod-publish-tickets-to-pubsub",
    task_id="prod_publish_tickets_to_pubsub",
    namespace="default",
    image="us.gcr.io/cityblock-data/zendesk:latest",
    image_pull_policy="Always",
    secrets=[zendesk_worker_svc_acct, zendesk_api_creds],
