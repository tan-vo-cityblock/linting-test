...
""" Runs necessary tasks to load in monthly data dump provided to CBH by Connecticare Amysis

Schedule: Every day
Workflow:
  1) [get_latest_drop_date] Find the most recent date that CCI Amysis has delivered files within `silver_tables`.
  ...
  4) [scio_transform_to_gold] Run PolishCCIClaimsData scio job for the given delivery date.
  ...
  6) [scio_combine_gold] Run CombineCCI to combine the shard produced by `scio_transform_to_gold` with
     the most recent shard in `gold_claims_merged`.
  7) [dbt_flatten_gold] Run `dbt` to populate `gold_claims_flattened` for the given delivery date.
  8) [run_great_expectations] Run `great_expectations` on `gold_claims_flatttened` for the given delivery date.
  9) [email_stakeholders] Inform stakeholders that silver_claims, gold_claims_incremental_amysis, 
...
partner_conf = LoadPartnerDataConfig(
    ...
    gold_dataset="gold_claims_incremental_amysis",
...
dag = DAG(
    dag_id="load_monthly_data_cci_amysis_v1",
...
bq_silver_shard_exists = KubernetesPodOperator(
    image=load_monthly_data_image,
    namespace="default",
    name="bq-silver-shard-exists",
    task_id="bq_silver_shard_exists",
    secrets=[secret_mount],
    env_vars={"GOOGLE_APPLICATION_CREDENTIALS": "/var/secrets/google/key.json"},
    image_pull_policy="Always",
    cmds=[
...
def run_scio_or_end(**context):
    found_bq_shard = context["task_instance"].xcom_pull(
        task_ids="bq_silver_shard_exists"
    )
...
get_latest_drop_date_facets = KubernetesPodOperator(
    image=load_monthly_data_image,
    namespace="default",
    name="get-latest-drop-date-facets",
    task_id="get_latest_drop_date_facets",
    secrets=[secret_mount],
    env_vars={"GOOGLE_APPLICATION_CREDENTIALS": "/var/secrets/google/key.json"},
    image_pull_policy="Always",
    cmds=["python", "./get_latest_drop_date.py", "--partner=cci_facets"],
    do_xcom_push=True,
    dag=dag,
)
...
combine_provider = KubernetesPodOperator(
    ...
    cmds=[
        ...
        "--facets_date={{ task_instance.xcom_pull(task_ids='get_latest_drop_date_facets') }}",
        "--amysis_date={{ task_instance.xcom_pull(task_ids='get_latest_drop_date') }}",
...
run_great_expectations_lab = KubernetesPodOperator(
    ...
    cmds=[
        ...
        "--data_group=labs",
        f"--partner={GREAT_EXPECTATIONS_PARTNER}",
        f"--bq_table={partner_conf.bq_project}.{partner_conf.combined_gold_dataset}.LabResult_"
        + "{{ task_instance.xcom_pull(task_ids='get_latest_drop_date') }}",
...
email_stakeholders = EmailOperator(
    ...
    subject="Airflow: Load Monthly CCI Amysis complete for shard: {{ task_instance.xcom_pull(task_ids='get_latest_drop_date') }}",
    html_content=partner_conf.result_email_html(
        description="Load monthly CCI Amysis",
        dag_id=dag.dag_id,
        shard="{{ task_instance.xcom_pull(task_ids='get_latest_drop_date') }}",
    ),
    dag=dag,
...
get_latest_drop_date >> bq_silver_shard_exists

# using presence of shard to determine whether or not we run scio job
bq_silver_shard_exists >> run_scio_or_end
...
scio_transform_to_gold >> [
    ...
    get_latest_drop_date_facets,
]
get_latest_drop_date_facets >> combine_provider
bq_get_latest_combined_gold_shard >> scio_combine_claims

# flatten and run great expectations and send notification email
scio_combine_claims >> [flatten_gold, email_stakeholders, run_great_expectations_lab]
