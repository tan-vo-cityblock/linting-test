...
dag = DAG(
    dag_id="publish_emblem_cohort_gold_data_v1",
    description="GCS -> BigQuery (gold_claims)",
    start_date=datetime.datetime(2020, 11, 1, 7 + 4),
...
publish_member_attribution = KubernetesPodOperator(
    ...
    cmds=[
        ...
        f"--bigqueryProject={PARTNER_PROJECT}",
        "--dataset=gold_claims",
...
dbt_run_cmd = """	
    dbt deps &&	
    dbt run --model source:emblem+ source:emblem_silver+ --exclude tag:nightly+	abstractions.computed
"""
...
email_stakeholders = EmailOperator(
    ...
    subject="Airflow: Emblem data published for shard: {{ task_instance.xcom_pull(task_ids='get_latest_drop_date') }}",
    html_content="Commons patient encounters, member attribution, and dbt tables have been updated with latest data",
    dag=dag,
)
...
# publish data
update_gold_views >> [push_claims_encounters, publish_member_attribution, dbt_run]

# email stakeholders
[push_claims_encounters, publish_member_attribution, dbt_run] >> email_stakeholders
