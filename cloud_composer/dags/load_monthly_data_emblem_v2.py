...
Workflow:
  ...
  5) [flatten_gold] Run flatten_gold.py to populate `gold_claims_flattened` for the given delivery date.
  6) [run_great_expectations] Run `great_expectations` on `gold_claims_flattened` for the given delivery date.
  7) [email_stakeholders] Inform stakeholders that silver_claims, gold_claims, and gold_claims_flattened
...
silver_config_paths = [f"emblem_silver/{table}.txt" for table in silver_tables]
...
dag = DAG(
    dag_id="load_monthly_data_emblem_v2",
...
scio_load_crosswalk = KubernetesPodOperator(
    ...
    cmds=[
        ...
        "--inputConfigPaths=emblem_silver/crosswalk.txt",
    ],
    dag=dag,
)

# TODO right now LoadToSilver relies on the partner configs to say which bucket to read from, but
#      testing would be way easier if that was configurable on the command line
...
email_stakeholders = EmailOperator(
    ...
    subject="Airflow: Load Monthly Emblem complete for shard: {{ task_instance.xcom_pull(task_ids='get_latest_drop_date') }}",
    html_content=partner_conf.result_email_html(
        description="Load monthly Emblem",
        dag_id=dag.dag_id,
        shard="{{ task_instance.xcom_pull(task_ids='get_latest_drop_date') }}",
    ),
    dag=dag,
...
# wait for both load jobs to finish before e-mailing stakeholders
[scio_load_crosswalk, scio_transform_to_gold] >> email_stakeholders

# flatten -> QA
scio_transform_to_gold >> flatten_gold
flatten_gold >> [
    run_great_expectations_facility,
    run_great_expectations_professional,
    run_great_expectations_lab,
]
