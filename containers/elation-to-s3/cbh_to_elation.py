...
class QueryToCloudStorageSender:  # TODO: Decouple this to generic SQL-->CSV, and file-->GCS
    """ Prepares data to be sent by running SQL queries on BQ and producing results as CSVs
     and storing them in GCS
    """

    HEADER_SUFFIX = 'header'

    def __init__(self, project_id: str, bucket_name: str, datestamp: str, temp_table_sleep: int):
        self.bq_client = bigquery.Client(project=project_id)
        self.storage_client = storage.Client(project=project_id)
        self.bucket = self.storage_client.bucket(bucket_name)
        self.datestamp = datestamp
        self.temp_table_sleep = temp_table_sleep

    def run(self, metrics_to_run: List[str], sub_dir: str) -> None:
        """ 'main' method to run relevant SQL metrics to deliver to GCS"""

        for metric in metrics_to_run:
            base_destination_uri = f"{sub_dir}/{self.datestamp}/{metric}"
            ...
            self.run_query_and_put_csv_in_gcs(metric, base_destination_uri, self.temp_table_sleep)
    ...
    def run_query_and_put_csv_in_gcs(self, metric_name: str, base_destination_uri, sleep_seconds: int) -> None:
        """ Runs a metric by running a SQL query on BQ then writing those results on GCS as a CSV file
        :param base_destination_uri: Destination string for the CSV file
        :param sleep_seconds: Number of seconds to wait to ensure the temp table is available
        ...
        time.sleep(sleep_seconds)  # delay to ensure the temporary table is available
...
def argument_parser():
    cli = argparse.ArgumentParser(
        description="Argument parser for Cityblock data to Elation")
...
if __name__ == "__main__":
    ...
    elation_sender = QueryToCloudStorageSender(
        project_id=bq_project, bucket_name=gcs_bucket, datestamp=date, temp_table_sleep=5)

    # TODO: Parameterize metrics and folder up at the orchestration level instead of this implementation
    ...
    folder = "elation_caregaps_drop"
    elation_sender.run(metrics_care_gaps, folder)
    logging.info(
        f"CSVs should now be available in bucket: {elation_sender.bucket.name}/{folder}/{date}"
    )
    ...
    folder = "elation_measures_drop"
    elation_sender.run(metrics_measures, folder)
    logging.info(
        f"CSVs should now be available in bucket: {elation_sender.bucket.name}/{folder}/{date}"
    )
