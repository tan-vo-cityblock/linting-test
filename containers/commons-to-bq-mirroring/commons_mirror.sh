...
while getopts "e:p:b:d:" OPTION; do
  case $OPTION in
...
if [ -z "${COMMONS_ENV}" ] || [ -z "${COMMONS_DATABASE}" ] || [ -z "${GCS_BUCKET_NAME}" ]];
  then usage
fi

echo "Running Commons mirror for [env: ${COMMONS_ENV}, database: ${COMMONS_DATABASE}, bucket: ${GCS_BUCKET_NAME}, date: ${DATE_SUFFIX}]"
...
DB_BACKUP_HANDLE="${COMMONS_DATABASE}-backup-${DATE_SUFFIX}"
...
aptible backup:restore "${LATEST_BACKUP_ID}" --handle ${DB_BACKUP_HANDLE}
...
RELEVANT_COMMONS_TABLES_QUERY="(SELECT tablename FROM pg_tables WHERE tablename NOT LIKE ALL(ARRAY['pg_%', 'sql_%', 'knex%', 'google_auth']))"
...
echo "Finished gcloud authenticantion to copy exported schemas and data"
