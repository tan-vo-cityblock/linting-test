...
from {{ ref('cm_del_patients_final_flags_quarterly') }}
group by 1,2
),
...
count(DISTINCT case when case_active_current_claims_quarter = true then pf.patientid else null end) as Total_members_active,
count(DISTINCT case when case_active_current_claims_quarter = true and hospitalized_current_claims_quarter = true
then pf.patientid else null end) as Total_members_hospitalized,
from {{ ref('cm_del_encounters_quarterly') }} encounters
left join {{ ref('cm_del_patients_final_flags_quarterly') }} pf
...
goals as (
	Select
		line_of_business,
    partner,
    count(distinct case when short_term_goal_closed = true then pf.patientid else null end) as Total_short_term_goals_closed,
	...
	From {{ ref('cm_del_member_action_plan_quarterly') }} member_action_plan
  left join {{ ref('cm_del_patients_final_flags_quarterly') }} pf
    on member_action_plan.patientid = pf.patientid
	Group by 1,2
),
--this to be YTD
users as (
...
cm_delegation_summary_quarterly as (
select
partner,
line_of_business,
total_attributed,
...
claims_completion_quarter_year as claims_reporting_quarter,
Total_members_hospitalized as Total_members_hospitalized_claims_quarter,
Total_members_active as Total_members_active_thru_claims_quarter,
...
select * from cm_delegation_summary_quarterly
