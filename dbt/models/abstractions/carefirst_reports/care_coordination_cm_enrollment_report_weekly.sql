...
--need to make sure we are taking their state until the last day of last week
with currentstate_last_week as (
  select
    patientid,
    historicalState,
    date(patientStateCreatedAt) as patientStateCreatedAtdate
  from  {{ ref('member_historical_states') }}
  where date_sub(date_trunc(current_date, week), interval 1 day) = calendardate
),
...
members as (
  ...
  left join {{ ref('member_states') }} ms
    using(patientId)
  left join currentstate_last_week using (patientid)
...
assessments as (
  ...
  where
    ...
    date(scoredAt) < date_trunc(current_date, week) and
    date(createdAt) < date_trunc(current_date, week)
  group by 1
),

-- any task counts as a "care plan". Last task created as of end of last week.
care_plans as (
  select
    patientId,
    max(date(taskCreatedAt)) as Date_of_Last_Care_Plan
  from {{ ref('member_goals_tasks') }}
  where date(taskCreatedAt) < date_trunc(current_date, week)
  group by 1
),
...
latest_attempted_connections as (
  ...
  where isAttemptedConnection is true
    and date(eventTimestamp) < date_trunc(current_date, week)
  group by patientId
),
...
reason as (
  ...
  where date(createdAt) < date_trunc(current_date, week)
),
