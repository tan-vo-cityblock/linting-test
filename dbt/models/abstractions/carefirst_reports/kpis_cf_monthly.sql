--this report is on everything from the previous month only
...
with historical_current_state as (
  ...
  where  last_day(date_sub(current_date, interval 1 month), month) = calendardate
),
...
disenrolled_firstday as (
  ...
  from {{ ref('member_historical_states') }}
  where DATE_TRUNC(date_sub(current_date, interval 1 month), month) = calendardate
),

---we want those we were attributed/consented/interested/enrolled between 3 months ago before last month and end of last month
member_state_dates as (
  select
    patientid,
    historicalState_lastday as historicalState,
    case when date(attributedat) <= last_day(date_sub(current_date, interval 1 month), month)
            then attributedat else null end as attributedat_until_lastday,
    case when date(attributedat) between date_trunc(date_sub(current_date, interval 1 month), month) and last_day(date_sub(current_date, interval 1 month), month)
    ...
    case when date(attributedat) between date_trunc(date_sub(current_date, interval 4 month), month) and last_day(date_sub(current_date, interval 1 month), month)
            then attributedat else null end as attributedat_90days,
    case when date(consentedat) between date_trunc(date_sub(current_date, interval 1 month), month) and last_day(date_sub(current_date, interval 1 month), month)
            and date(attributedat) between date_trunc(date_sub(current_date, interval 1 month), month) and last_day(date_sub(current_date, interval 1 month), month)
            and historicalState_lastday not in ('disenrolled_after_consent', 'disenrolled')
            then consentedat else null end as consentedat_within_time,
    case when date(consentedat) between date_trunc(date_sub(current_date, interval 4 month), month) and last_day(date_sub(current_date, interval 1 month), month)
            then consentedat else null end as consentedat_90days,
    case when date(enrolledat) between date_trunc(date_sub(current_date, interval 1 month), month) and last_day(date_sub(current_date, interval 1 month), month)
            and date(attributedat) between date_trunc(date_sub(current_date, interval 1 month), month) and last_day(date_sub(current_date, interval 1 month), month)
            and historicalState_lastday not in ('disenrolled_after_consent', 'disenrolled')
            then enrolledat else null end as enrolledat_within_time,
    case when date(enrolledat) between date_trunc(date_sub(current_date, interval 4 month), month) and last_day(date_sub(current_date, interval 1 month), month)
  ...
  --for contacted attempted we want anyone attributed in that time period with ANY contacted attempted (not specific to time period)
    ...
    case when date(interestedat) between date_trunc(date_sub(current_date, interval 1 month), month) and last_day(date_sub(current_date, interval 1 month), month)
            and date(attributedat) between date_trunc(date_sub(current_date, interval 1 month), month) and last_day(date_sub(current_date, interval 1 month), month)
            and historicalState_lastday not in ('disenrolled_after_consent', 'disenrolled')
            then interestedat else null end as interestedat_within_time,
    case when date(interestedat) between date_trunc(date_sub(current_date, interval 4 month), month) and last_day(date_sub(current_date, interval 1 month), month)
...
assessments_completed_attributed_consented as (
  select
    ...
    count(distinct case when
          ...
          then patientid else null end) as assessments_completed_consented_or_interested_or_enrolled_90days
  from {{ ref('member_commons_completion') }} mcc
  left join member_state_dates
    using(patientId)
  where minComprehensiveAssessmentAt is not null
  and date(minComprehensiveAssessmentAt) <= last_day(date_sub(current_date, interval 1 month), month)
  group by 1
),
...
assessments_completed_attributed_consented_hh as (
  select
    ...
    count(distinct case when
          ...
          then patientid else null end) as assessments_completed_consented_or_interested_or_enrolled_90days_hh
  from {{ ref('hh_enrolled_exclusion') }} hh
...
members_completed_assessment_30days as (
  ...
  from  {{ ref('member_commons_completion') }} mcc
  where minComprehensiveAssessmentAt is not null
  and date(minComprehensiveAssessmentAt) between date_trunc(date_sub(current_date, interval 2 month), month) and last_day(date_sub(current_date, interval 2 month), month)
  and date(mintaskat) <= last_day(date_sub(current_date, interval 1 month), month)
),
...
--want to get two weeks before the 1st of the month and 2 weeks after the last day of the month
and date(attemptedAt) between date_sub(date_trunc(date_sub(current_date, interval 1 month), month), interval 2 week) and date_add(last_day(date_sub(current_date, interval 1 month), month), interval 2 week)
...
"without_hh" as without_hh,
 --Cohort members with attempted contact: #members have had a contacted attempt not disenrolled/members newly attributed in time period not disenrolled
 ...
 ----#members of members interested, consented, or enrolled not disenrolled/members newly attributed in time period not disenrolled
 ...
 ----#of members with HRA assessment within 90days of consented or enrolled/total members consented/enrolled/interested
    sum(assessments_completed_consented_or_interested_or_enrolled_90days_hh) as assessments_completed_consented_or_enrolled_90days_hh,
    ...
    round(((sum(assessments_completed_consented_or_interested_or_enrolled_90days_hh)/nullif(sum(total_members_enrolled_or_consented_or_interested_90days_hh),0))*100),1) as hra_completed_within90_days_enrolled_or_consented_or_interested_hh,
...
"with_hh" as with_hh,
 ...
 --Assessment completion within ninety (90) days of Cohort Members engaged and/or consenting for Care Management:
 ----#of members with HRA assessment within 90days of consented or enrolled/total members consented/enrolled/interested
    sum(assessments_completed_consented_or_interested_or_enrolled_90days) as assessments_completed_consented_or_enrolled_90days,
    sum(total_members_enrolled_or_consented_or_interested_90days) as total_members_enrolled_or_consented_or_interested_90days,
    round(((sum(assessments_completed_consented_or_interested_or_enrolled_90days)/nullif(sum(total_members_enrolled_or_consented_or_interested_90days),0))*100),1) as hra_completed_within90_days_enrolled_or_consented_or_interested
from  number_members
