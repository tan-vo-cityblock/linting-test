...
with assessed as (
    select
        *,
        ...
        ---hard to reach based on unreach logic: Unreachable: 120+ Days, Past Initial Onboarding & never reached
         coalesce((date_diff(date(reporting_datetime_last),cohort_or_attributed_date, day) > 120
    ...
    From {{ ref('cm_del_patients_monthly') }}
),
...
case
   when consentedat is not null then date_diff(date(consentedAt), reporting_date_last,day)
