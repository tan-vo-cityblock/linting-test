...
with map AS (
  ...
  case when g.deletedAt between  rd.reporting_datetime_first and rd.reporting_datetime_last then g.deletedAt else null end as goalDeletedAt,
  case when g.completedAt between  rd.reporting_datetime_first and rd.reporting_datetime_last then g.completedAt else null end as goalCompletedAt,
  case when g.dueAt between  date(rd.reporting_datetime_first) and date(rd.reporting_datetime_last) then g.dueAt else null end as goalDueAt,
  t.groupId as taskGroupId,
  t.id as taskId,
  t.title as taskTitle,
  t.createdAt as taskCreatedAt,
  case when t.deletedAt between  rd.reporting_datetime_first and rd.reporting_datetime_last then t.deletedAt else null end as taskDeletedAt,
  case when t.completedAt between  rd.reporting_datetime_first and rd.reporting_datetime_last then t.completedAt else null end as taskCompletedAt,
  case when t.dueAt between  date(rd.reporting_datetime_first) and date(rd.reporting_datetime_last) then t.dueAt else null end as taskDueAt,
...
--Other dates act the same but if they are after the reporting date, they are null
  INNER join {{ ref('cm_del_reporting_dates_monthly') }} rd
    on  g.createdAt between  rd.reporting_datetime_first and rd.reporting_datetime_last
    or  t.createdAt between  rd.reporting_datetime_first and rd.reporting_datetime_last
