...
base_event_data as (
  ...
  left join `emblem-data.silver_claims.providers` as p
    on f.ATT_PROV = p.provider.PROV_ID and
    f.SERVICINGPROVLOCSUFFIX = p.provider.PROV_LOC

  where patientId is not null and
    bucket IN ('ED', 'Acute Inpatient', 'Inpatient Psych', 'Inpatient Rehab', 'SNF')
...
final as (
  ...
  select
    STRUCT(
        ...
        'eh_all_claims_may_19_f' as table
      ) as source,
    ...
    STRUCT(
      ...
      case when bucket = 'ED' then 'ED'
           ...
           when REGEXP_CONTAINS(bucket, 'SNF') then 'SNF'
    ...
    [
      STRUCT(
        ...
        case when bucket IN ('ED', 'Acute Inpatient') then 'acute'
        else bucket end as `key`
      )
    ] as purposes,
