...
with map AS (
  ...
  case when g.updatedAt <= reporting_datetime then g.updatedAt else null end as goalUpdatedAt,
  case when g.deletedAt <= reporting_datetime then g.deletedAt else null end as goalDeletedAt,
  case when g.completedAt <= reporting_datetime then g.completedAt else null end as goalCompletedAt,
  case when g.dueAt <= reporting_date then g.dueAt else null end as goalDueAt,
  t.groupId as taskGroupId,
  t.id as taskId,
  t.title as taskTitle,
  t.createdAt as taskCreatedAt,
  case when t.updatedAt <= reporting_datetime then t.updatedAt else null end as taskUpdatedAt,
  case when t.deletedAt <= reporting_datetime then t.deletedAt else null end as taskDeletedAt,
  case when t.completedAt <= reporting_datetime then t.completedAt else null end as taskCompletedAt,
  case when t.dueAt <= reporting_date then t.dueAt else null end as taskDueAt,
...
--Other dates act the same but if they are after the reporting date, they are null
  INNER join {{ ref('cm_del_reporting_dates_ytd') }}
    on  g.createdAt <= reporting_datetime
    or  t.createdAt <= reporting_datetime
