...
initials2 as (
  ...
  -- Days in CM calc
  abs(case when case_active_thru_period is true then date_diff(reporting_quarter_end,date(consented_at),day)
  ...
  From  {{ ref('cm_del_kpi_ytd') }} kpi
  left join initials using (patientid,partner,Line_of_business,reporting_quarter_year)
...
claims_completion_month as (
  ...
  min(claims_completion_quarter_year) as cpy
  from initials2
...
days_in_cm_thru_period_cat as (
  ...
  days_in_cm.reporting_quarter_year,
  days_in_cm.reporting_quarter_year as rpy,
...
left join days_in_cm_thru_period_cat cat
  ...
  and kpi.event_quarter_year = cat.reporting_quarter_year
  and kpi.partner = cat.partner
  and kpi.line_of_business = cat.line_of_business
left join initials2
  on kpi.patientid = initials2.patientid
  and kpi.event_quarter_year = initials2.reporting_quarter_year
...
From  {{ ref('cm_del_kpi_ytd') }} kpi
left join initials2 using (patientid,partner,Line_of_business,reporting_quarter_year)
left join unable using (patientid,partner,Line_of_business,reporting_quarter_year)
group by 1,2,3
),
...
left join non_cumulative2 f2 using (partner, line_of_business, event_quarter_year)
),
...
final_cumulative.reporting_quarter_year,
total_cases,
...
left join final_non_cumulative
    on final_cumulative.reporting_quarter_year = final_non_cumulative.event_quarter_year
...
left join averages
    on final_cumulative.reporting_quarter_year = averages.reporting_quarter_year
