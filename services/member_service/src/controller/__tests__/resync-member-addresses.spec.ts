...
import { resyncMemberAddresses } from '../resync-member-addresses';
import { resyncAddressesPubSubMessage } from './test-data';
import { setupDb } from './test-setup';

const sandbox = sinon.createSandbox();
describe('Resync member addresses route', () => {
  ...
  describe('on success', () => {
    ...
    it('should resync member addresses', async () => {
      const memberId = 'a3ccab32-d960-11e9-b350-acde48001122';
      const response = httpMocks.createResponse();
      const request = httpMocks.createRequest({
        method: 'GET',
        url: `/${API_VERSION}/members/resync/${memberId}/demographics/addresses`,
        params: { memberId }
      });
      ...
      await resyncMemberAddresses(request, response, console.log);

      const testData = stringifyMessage(resyncAddressesPubSubMessage);
  ...
  describe('on failure', () => {
    ...
    it('should fail to publish a member to pubsub if member does not exist', async () => {
      ...
      // tslint:disable-next-line:no-console
      await resyncMemberAddresses(request, response, next);
