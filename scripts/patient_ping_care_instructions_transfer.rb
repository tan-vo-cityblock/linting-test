#! /usr/bin/env ruby
#
# This script loads Patient Ping care instructions information from BigQuery
...
QUERY = File.read(File.join(File.dirname(__FILE__), 'patient_ping_care_instructions.sql'))

GCP_PROJECT = "cbh-db-mirror-prod"
FILE_PATH = ENV["FILE_PATH"] || "/tmp/patient_ping_care_instructions.csv"
...
begin
  if results.length > 0
    ...
    Net::SFTP.start('put.patientping.com', 'cityblock', password: ENV["PATIENT_PING_PASSWORD"], port: '22999', verify_host_key: :always) do |sftp|
      sftp.upload!(FILE_PATH, "/in/cityblock_care_instructions.csv")
    end
  end
ensure
